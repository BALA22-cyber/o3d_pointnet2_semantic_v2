...
# from tf_ops.tf_grouping import query_ball_point, group_point, knn_point ## original
# from tf_ops.tf_sampling import farthest_point_sample, gather_point #original
from tf_ops.tf_grouping import group_point, knn_point
from tf_ops.tf_sampling import gather_point
from util.alter_utils import query_ball_point, farthest_point_sample
...
def sample_and_group(npoint, radius, nsample, xyz, points, knn=False, use_xyz=True):
    ...
    new_xyz = gather_point(
        xyz, farthest_point_sample(xyz,npoint) 
        # xyz, farthest_point_sample(npoint, xyz) original
    )  # (batch_size, npoint, 3)
    if knn:
...
def pointnet_sa_module(
    ...
    with tf.compat.v1.variable_scope(scope) as sc:
        ...
        if use_nchw:
            new_points = tf.transpose(a=new_points, perm=[0, 3, 1, 2])
        ...
        if pooling == "max":
            new_points = tf.reduce_max(
                input_tensor=new_points, axis=[2], keepdims=True, name="maxpool"
            )
        elif pooling == "avg":
            new_points = tf.reduce_mean(
                input_tensor=new_points, axis=[2], keepdims=True, name="avgpool"
            )
            ...
            with tf.compat.v1.variable_scope("weighted_avg"):
                ...
                weights = exp_dists / tf.reduce_sum(
                    input_tensor=exp_dists, axis=2, keepdims=True
...
def pointnet_sa_module_msg(xyz,points,npoint,radius_list,nsample_list,mlp_list,is_training,bn_decay,scope,bn=True,use_xyz=True,use_nchw=False,):
    ...
    data_format = "NCHW" if use_nchw else "NHWC"
    with tf.compat.v1.variable_scope(scope) as sc:
        # new_xyz = gather_point(xyz, farthest_point_sample(npoint, xyz)) original
        new_xyz = gather_point(xyz, farthest_point_sample(xyz,npoint))
        new_points_list = []

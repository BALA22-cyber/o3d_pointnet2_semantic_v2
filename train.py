...
def get_learning_rate(batch):
    ...
    learning_rate = tf.keras.optimizers.schedules.ExponentialDecay(
        ...
        decay_steps=float(PARAMS["decay_step"]),  # Decay step.
        decay_rate=PARAMS["learning_rate_decay_rate"],  # Decay rate.
        staircase=True,
    )
...
def get_bn_decay(batch):
    ...
    bn_momentum = tf.keras.optimizers.schedules.ExponentialDecay(
        PARAMS["bn_init_decay"],  # Base learning rate.
        #batch * PARAMS["batch_size"],  # Current index into the dataset.
        decay_steps=float(PARAMS["decay_step"]),  # Decay step.
        decay_rate=PARAMS["bn_decay_decay_rate"],  # Decay rate.
        staircase=True,
    )
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        with tf.device("/gpu:" + str(PARAMS["gpu"])):
            # pointclouds_pl, labels_pl, smpws_pl = model.get_placeholders(  ### original
            #     PARAMS["num_point"], hyperparams=PARAMS
            # )
            pointclouds_pl, labels_pl, smpws_pl = model.placeholder_inputs( PARAMS["batch_size"] ,
            PARAMS["num_point"], hyperparams=PARAMS
            )
            
            is_training_pl = tf.keras.Input(dtype=tf.bool, shape=())    
            ...
            accuracy = tf.reduce_sum(input_tensor=tf.cast(correct, tf.float32)) / float(PARAMS["batch_size"] * PARAMS["num_point"])
            tf.summary.scalar("accuracy", accuracy)

            # Computer mean intersection over union
            mean_intersection_over_union, update_iou_op = tf.compat.v1.metrics.mean_iou(
                tf.cast(labels_pl, dtype=tf.int32), tf.cast(tf.argmax(input=pred, axis=2), dtype=tf.int32), NUM_CLASSES)
            tf.summary.scalar("mIoU", tf.cast(mean_intersection_over_union, dtype=tf.float32))
        ...
        validation_writer = tf.summary.FileWriter(os.path.join(PARAMS["logdir"], "validation"), sess.graph)

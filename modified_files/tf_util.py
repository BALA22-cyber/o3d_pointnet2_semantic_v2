...
def conv1d(
  ...
  """
    with tf.compat.v1.variable_scope(scope) as sc:
        ...
        if data_format == "NHWC":
            # num_in_channels = inputs.get_shape()[-1].value # tf1
        ...
        outputs = tf.nn.conv1d(
            input=inputs, filters=kernel, stride=stride, padding=padding, data_format=data_format_1d
        )
        biases = _variable_on_cpu(
            "biases", [num_output_channels], tf.compat.v1.constant_initializer(0.0)
        )
...
def fully_connected(
  ...
  """
    with tf.compat.v1.variable_scope(scope) as sc:
        ...
        biases = _variable_on_cpu("biases", [num_outputs], tf.compat.v1.constant_initializer(0.0))
        outputs = tf.nn.bias_add(outputs, biases)
  ...
  """
    with tf.compat.v1.variable_scope(scope) as sc:
        ...
        beta = _variable_on_cpu(
            name="beta", shape=[num_channels], initializer=tf.compat.v1.constant_initializer(0)
        )
        gamma = _variable_on_cpu(
            name="gamma", shape=[num_channels], initializer=tf.compat.v1.constant_initializer(1.0)
        )
...
#     # return tf.contrib.layers.batch_norm( # tf1
...
def batch_norm_template(inputs, is_training, scope, moments_dims_unused, bn_decay, data_format="NHWC"):
    ...
    Args:
        ...
        scope: string, variable scope (not used in this implementation)
        moments_dims_unused: a list of ints, indicating dimensions for moments calculation (not used in this implementation)
    ...
    Return:
        normed: batch-normalized maps
    """
    bn_decay_value = 0.9 if bn_decay is None or isinstance(bn_decay, tf.Tensor) else float(bn_decay)

    # Determine the axis based on the data format
    axis = -1 if data_format == "NHWC" else 1

    batch_norm_layer = tf.keras.layers.BatchNormalization(
        axis=axis,
        momentum=bn_decay_value,
        center=True,
        scale=True,
    )

    # Use tf.cond to handle the training argument correctly in graph mode
    def apply_batch_norm_training():
        return batch_norm_layer(inputs, training=True)
    
    def apply_batch_norm_inference():
        return batch_norm_layer(inputs, training=False)
    
    return tf.cond(is_training, apply_batch_norm_training, apply_batch_norm_inference)
...
def dropout(inputs, keep_prob, is_training, scope, noise_shape=None):
    """ Dropout layer.
    ...
    Args:
        ...
        is_training: boolean tf.Variable, true indicates training phase.
        scope: string, variable scope.
        noise_shape: 1D tensor of type int32, representing the shape for randomly generated keep/drop flags.
    
    Return:
        outputs: Tensor, output after dropout.
    """
    rate = 1 - keep_prob
    with tf.name_scope(scope):
        outputs = tf.cond(
            is_training,
            true_fn=lambda: tf.nn.dropout(inputs, rate=rate, noise_shape=noise_shape),
            false_fn=lambda: inputs
        )
    return outputs

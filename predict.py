...
class Predictor:
    def __init__(self, checkpoint_path, num_classes, hyper_params):
        ...
        with tf.device("/gpu:0"):
            ...
            pl_is_training = tf.keras.Input(dtype=tf.bool, shape=())
            print("pl_points shape", tf.shape(input=pl_points))
            ...
            pl_sparse_points = tf.keras.Input(dtype=tf.float32, shape=(None, 3))
            pl_sparse_labels = tf.keras.Input(dtype=tf.int32, shape=(None,))
            pl_dense_points = tf.keras.Input(dtype=tf.float32, shape=(None, 3))
            pl_knn = tf.keras.Input(dtype=tf.int32, shape=())
...
if __name__ == "__main__":
    ...
    # Create output dir
    output_dir = os.path.join("log_0.1/result", "sparse")

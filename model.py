...
#     pointclouds_pl = tf.placeholder(tf.float32, shape=(None, num_point, 3 + feature_size))
...
def placeholder_inputs(batch_size, num_point, hyperparams):
    ...
    pointclouds_pl = tf.keras.Input(dtype=tf.float32, shape=(batch_size, num_point, 3+feature_size))
    labels_pl = tf.keras.Input(dtype=tf.int32, shape=(batch_size, num_point))
    smpws_pl = tf.keras.Input(dtype=tf.float32, shape=(batch_size, num_point))
    return pointclouds_pl, labels_pl, smpws_pl
...
def get_loss(pred, label, smpw, end_points):
    ...
    # )
    classify_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
        labels=label, logits=pred
    )
    tf.summary.scalar("classify loss", classify_loss)
    tf.Graph.add_to_collection("losses", classify_loss)

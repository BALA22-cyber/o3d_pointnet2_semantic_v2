...
class PredictInterpolator:
    def __init__(self, checkpoint_path, num_classes, hyper_params):
        ...
        with tf.device("/gpu:0"):
            ...
            pl_is_training = tf.compat.v1.placeholder(tf.bool, shape=())
            ...
            sparse_labels_batched = tf.argmax(input=pred, axis=2)
            ...
            pl_sparse_points_batched = tf.compat.v1.placeholder(tf.float32, (None, None, 3))
            sparse_points = tf.reshape(pl_sparse_points_batched, [-1, 3])
            pl_dense_points = tf.compat.v1.placeholder(tf.float32, (None, 3))
            pl_knn = tf.compat.v1.placeholder(tf.int32, ())
